...
import { BarChart3, Coins, CreditCard, Eye, EyeOff, Home, Landmark, Lightbulb, Plus, RefreshCw, Save, Sparkles, UserCircle, Wallet, Waves, X } from 'lucide-react';
...
type Tab = 'accounts' | 'coach' | 'portfolio';
...
const typeIcon = { checking: Landmark, savings: Landmark, cash: Wallet, investment: BarChart3, crypto: Coins, retirement: Landmark, property: Home, loan: CreditCard, credit: CreditCard };
...
export default function VaultDashboard() {
  ...
  const [coachMode, setCoachMode] = useState<'ai' | 'rules'>('rules');
  ...
  async function loadData() {
    ...
    if (!fullResult.error && fullResult.data) setAccounts(fullResult.data as Account[]);
    else {
      const basicResult = await supabase.from('accounts').select('id,name,institution,type,balance,currency').order('created_at', { ascending: false });
      if (!basicResult.error && basicResult.data) setAccounts(basicResult.data as Account[]);
    }
  ...
  useEffect(() => { loadData(); }, []);
  useEffect(() => { setCoach(buildPortfolioCoach(accounts)); }, [accounts]);
  ...
  const liquidPercent = pct(totals.cash + totals.investments + totals.crypto, totals.assets);
  ...
  const latestChange = useMemo(() => {
    ...
    const valueAt = (date: string) => accounts.reduce((sum, account) => {
      ...
      return sum + (['loan', 'credit'].includes(account.type) ? -Math.abs(amount) : amount);
    }, 0);
  ...
  async function refreshCoach() {
    ...
    try {
      ...
      setCoachMode(data.mode === 'ai' ? 'ai' : 'rules');
    } catch {
  ...
  useEffect(() => {
    if (tab === 'coach' && !coachTried && accounts.length) {
      setCoachTried(true);
      refreshCoach();
    }
  }, [tab, coachTried, accounts.length]);
  ...
  async function refreshQuickRate() {
    ...
    try { const result = await getMyrRate(quickCurrency); setQuickRate(String(result.rate)); setQuickRateMeta(`${result.source} · ${prettyRateTime(result.updatedAt)}`); }
    catch (error) { setStatus(error instanceof Error ? error.message : 'Unable to refresh rate.'); setQuickRateMeta('Manual rate'); }
  ...
  async function saveQuickUpdate() {
    ...
    let update = await supabase.from('accounts').update({ balance: quickMyr, currency: quickCurrency, native_balance: Number(quickNative), fx_rate: Number(quickRate) }).eq('id', quickAccount.id);
    if (update.error && update.error.message.toLowerCase().includes('native_balance')) update = await supabase.from('accounts').update({ balance: quickMyr, currency: quickCurrency }).eq('id', quickAccount.id);
    ...
    let snapshot = await supabase.from('account_snapshots').insert({ user_id: userId, account_id: quickAccount.id, snapshot_date: quickDate, balance: quickMyr, native_balance: Number(quickNative), currency: quickCurrency, fx_rate: Number(quickRate), notes: quickNote || 'Quick update' });
    ...
    setQuickAccount(null); setStatus(`${quickAccount.name} updated.`); await loadData();
  }
  async function snapshotToday() {
    ...
    if (result.error && result.error.message.toLowerCase().includes('native_balance')) result = await supabase.from('account_snapshots').insert(accounts.map((account) => ({ user_id: userId, account_id: account.id, snapshot_date: date, balance: Number(account.balance), notes: 'Daily snapshot' })));
    setSnapshotSaving(false); setStatus(result.error ? result.error.message : `Today's snapshot updated for ${accounts.length} accounts.`); await loadData();
  }
