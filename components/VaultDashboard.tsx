...
function signedValue(account: VaultAccount) { const value = Math.abs(Number(account.balance || 0)); return isDebt(account) ? -value : value; }
...
export default function VaultDashboard() {
  ...
  useEffect(() => {
    ...
    function onScroll() {
      ...
      const nearBottom = window.innerHeight + y > document.documentElement.scrollHeight - 170;
  ...
  const netWorthHistory = useMemo(() => {
    ...
    return dates.map((date) => {
      const value = accounts.reduce((sum, account) => {
        ...
        const amount = latest ? Math.abs(Number(latest.balance)) : Math.abs(Number(account.balance || 0));
        return sum + (isDebt(account) ? -amount : amount);
      }, 0);
      return { date, value };
    });
  }, [accounts, snapshots]);

  const latestChange = netWorthHistory.length > 1 ? netWorthHistory[netWorthHistory.length - 1].value - netWorthHistory[netWorthHistory.length - 2].value : 0;
  const chart = useMemo(() => {
    ...
    const coords: ChartPoint[] = source.map((point, index) => ({ ...point, x: source.length === 1 ? 350 : 35 + (index * 630) / (source.length - 1), y: 150 - ((point.value - min) / range) * 105 }));
    return { coords, path: coords.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x} ${p.y}`).join(' '), max, min };
  ...
  async function refreshCoach() {
    ...
    try { const response = await fetch('/api/coach', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ accounts }) }); const data = await response.json() as CoachResponse; setCoach(Array.isArray(data.actions) ? data.actions : buildPortfolioCoach(accounts)); setCoachMode(data.mode === 'ai' ? 'ai' : 'local'); setCoachMessage(data.mode === 'ai' ? `AI mode active. ${COACH_DAILY_LIMIT - count - 1} AI refreshes left today.` : data.error || 'AI unavailable. Local review is active.'); }
