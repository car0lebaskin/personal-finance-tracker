...
import { getBreakdown, getRecommendations, getTotals, nativeDisplay } from '@/lib/finance';
...
const typeIcon: Record<string, ElementType> = { checking: Landmark, savings: Landmark, cash: Wallet, investment: BarChart3, crypto: Coins, retirement: Landmark, property: Home, loan: CreditCard, credit: CreditCard };
...
export default function VaultDashboard() {
  ...
  const [quickAccount, setQuickAccount] = useState<Account | null>(null);
  ...
  const hidden = privacyMode ? '••••••' : null;
  const quickMyr = Number(quickNative || 0) * Number(quickRate || 0);

  async function loadData() {
    const session = await supabase.auth.getSession();
    if (!session.data.session) { router.push('/login'); return; }
    let accountResult = await supabase.from('accounts').select('id,name,institution,type,balance,currency,native_balance,fx_rate').order('created_at', { ascending: false });
    if (accountResult.error && accountResult.error.message.toLowerCase().includes('native_balance')) {
      accountResult = await supabase.from('accounts').select('id,name,institution,type,balance,currency').order('created_at', { ascending: false });
    }
  ...
  useEffect(() => {
    ...
    function onScroll() {
      ...
      if (nearBottom || y > lastY + 8) setShowAdd(false);
      if (y < lastY - 8 || y < 80) setShowAdd(true);
      lastY = y;
    }
  ...
  const netWorthHistory = useMemo(() => {
    ...
    return dates.map((date) => {
      ...
      accounts.forEach((account) => {
        ...
        const latest = list.filter((s) => s.snapshot_date <= date)[list.filter((s) => s.snapshot_date <= date).length - 1];
  ...
  const chart = useMemo(() => {
    const source = netWorthHistory.length ? netWorthHistory : [{ date: 'Current', value: totals.netWorth }];
    const max = Math.max(...source.map((p) => p.value), 1);
    const min = Math.min(...source.map((p) => p.value), 0);
    const range = Math.max(max - min, 1);
    const coords: ChartPoint[] = source.map((point, index) => ({ ...point, x: source.length === 1 ? 350 : 30 + (index * 640) / (source.length - 1), y: 130 - ((point.value - min) / range) * 90 }));
    return { coords, path: coords.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x} ${p.y}`).join(' ') };
  }, [netWorthHistory, totals.netWorth]);

  const selectedIndex = activeChartIndex ?? Math.max(chart.coords.length - 1, 0);
  const selectedPoint = chart.coords[selectedIndex];
  const previousPoint = selectedIndex > 0 ? chart.coords[selectedIndex - 1] : null;
  ...
  function openQuickUpdate(account: Account) {
    setQuickAccount(account);
    setQuickCurrency(account.currency || 'MYR');
    setQuickNative(String(account.native_balance ?? account.balance ?? 0));
    setQuickRate(String(account.fx_rate ?? 1));
    setQuickDate(today());
    setQuickNote('');
    setQuickRateMeta(account.currency === 'MYR' ? 'MYR base currency' : 'Saved rate');
  }

  async function refreshQuickRate() {
    if (quickCurrency === 'MYR') return;
    setQuickRateLoading(true); setStatus('');
    try {
      const result = await getMyrRate(quickCurrency);
      setQuickRate(String(result.rate));
      setQuickRateMeta(`${result.source} · ${prettyRateTime(result.updatedAt)}`);
    } catch (err) {
      setStatus(err instanceof Error ? err.message : 'Unable to refresh rate.');
      setQuickRateMeta('Manual rate');
    } finally { setQuickRateLoading(false); }
  }

  async function saveQuickUpdate() {
    if (!quickAccount) return;
    setQuickSaving(true); setStatus('');
    const session = await supabase.auth.getSession();
    const userId = session.data.session?.user.id;
    if (!userId) { router.push('/login'); return; }
    const fullUpdate = { balance: quickMyr, currency: quickCurrency, native_balance: Number(quickNative), fx_rate: Number(quickRate) };
    const fallbackUpdate = { balance: quickMyr, currency: quickCurrency };
    let update = await supabase.from('accounts').update(fullUpdate).eq('id', quickAccount.id);
    if (update.error && update.error.message.toLowerCase().includes('native_balance')) update = await supabase.from('accounts').update(fallbackUpdate).eq('id', quickAccount.id);
    if (update.error) { setQuickSaving(false); setStatus(update.error.message); return; }
    await supabase.from('account_snapshots').delete().eq('account_id', quickAccount.id).eq('snapshot_date', quickDate);
    const fullSnapshot = { user_id: userId, account_id: quickAccount.id, snapshot_date: quickDate, balance: quickMyr, native_balance: Number(quickNative), currency: quickCurrency, fx_rate: Number(quickRate), notes: quickNote || 'Quick update' };
    let snapshot = await supabase.from('account_snapshots').insert(fullSnapshot);
    if (snapshot.error && snapshot.error.message.toLowerCase().includes('native_balance')) snapshot = await supabase.from('account_snapshots').insert({ user_id: userId, account_id: quickAccount.id, snapshot_date: quickDate, balance: quickMyr, notes: quickNote || 'Quick update' });
    setQuickSaving(false);
    if (snapshot.error) { setStatus(snapshot.error.message); return; }
    setQuickAccount(null);
    setStatus(`${quickAccount.name} updated.`);
    await loadData();
  }
  ...
  async function snapshotToday() {
    ...
    const fullRows = accounts.map((account) => ({ user_id: userId, account_id: account.id, snapshot_date: date, balance: Number(account.balance), native_balance: Number(account.native_balance ?? account.balance), currency: account.currency || 'MYR', fx_rate: Number(account.fx_rate ?? 1), notes: 'Daily snapshot' }));
    let result = await supabase.from('account_snapshots').insert(fullRows);
    if (result.error && result.error.message.toLowerCase().includes('native_balance')) {
      result = await supabase.from('account_snapshots').insert(accounts.map((account) => ({ user_id: userId, account_id: account.id, snapshot_date: date, balance: Number(account.balance), notes: 'Daily snapshot' })));
    }
  ...
  return <main className="min-h-screen bg-[#080b08] text-[#f4f5ef]"><div className="mx-auto max-w-[680px] min-h-screen relative overflow-hidden pb-32"><div className="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_50%_-10%,rgba(148,255,97,0.12),transparent_34%),linear-gradient(180deg,#182016_0%,#080b08_42%)]"/><div className="relative px-4 pt-6"><header className="flex items-center justify-between mb-6"><h1 className="text-xl font-semibold tracking-tight">{tab === 'accounts' ? 'Vault' : tab === 'insights' ? 'Insights' : 'Portfolio'}</h1><div className="flex items-center gap-2"><button onClick={() => setPrivacyMode((value) => !value)} className="h-9 w-9 rounded-full bg-white/[0.06] border border-white/10 flex items-center justify-center" aria-label={privacyMode ? 'Show balances' : 'Hide balances'}>{privacyMode ? <EyeOff className="h-5 w-5"/> : <Eye className="h-5 w-5"/>}</button><button onClick={() => router.push('/accounts/profile')} className="h-9 w-9 rounded-full bg-white/[0.06] border border-white/10 flex items-center justify-center"><UserCircle className="h-6 w-6"/></button></div></header>{privacyMode && <div className="mb-3 rounded-2xl border border-[#a7ff4f]/20 bg-[#a7ff4f]/10 px-3 py-2 text-xs text-[#d8ded2]">Privacy mode is on. Tap the eye to reveal balances.</div>}<div className={cn('transition-all duration-300', privacyMode && 'blur-sm select-none pointer-events-none')}>{tab === 'accounts' && <section><p className="text-[#a8aca3] text-xs mb-1.5">Net worth</p><div className="text-[1.7rem] leading-tight font-light tracking-tight mb-2">{hidden || money(totals.netWorth)}</div><p className="text-[11px] text-[#a8aca3] mb-4">{netWorthHistory.length ? 'Based on saved account snapshots' : 'Add dated updates to build history'}</p><div className="h-[170px] -mx-4 mb-4 border-b border-white/10 relative"><svg viewBox="0 0 700 150" className="h-full w-full"><defs><linearGradient id="line" x1="0" x2="1"><stop offset="0%" stopColor="#35bdf5"/><stop offset="100%" stopColor="#69f0c2"/></linearGradient></defs><path d={chart.path} fill="none" stroke="url(#line)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>{chart.coords.map((p, index) => <circle key={`${p.date}-${index}`} onClick={() => setActiveChartIndex(index)} cx={p.x} cy={p.y} r={index === selectedIndex ? 7 : 4} fill={index === selectedIndex ? '#f4f5ef' : '#69f0c2'} className="cursor-pointer"/>)}</svg>{selectedPoint && <div className="absolute left-4 top-3 rounded-2xl bg-black/45 border border-white/10 px-3 py-2 backdrop-blur"><p className="text-[11px] text-[#a8aca3]">{prettyDate(selectedPoint.date)}</p><p className="text-sm font-mono">{hidden || money(selectedPoint.value)}</p><p className={selectedChange >= 0 ? 'text-[11px] text-[#75efad]' : 'text-[11px] text-red-200'}>{previousPoint ? `${selectedChange >= 0 ? '+' : ''}${hidden || money(selectedChange)} (${selectedPct.toFixed(1)}%)` : 'First snapshot'}</p></div>}</div><button onClick={snapshotToday} disabled={snapshotSaving} className="w-full rounded-2xl bg-[#a7ff4f] text-[#071006] font-semibold text-sm py-3 mb-2 disabled:opacity-60">{snapshotSaving ? 'Saving snapshot...' : 'Snapshot today'}</button>{status && <p className="text-xs text-[#a8aca3] mb-3">{status}</p>}<div className="mb-4 overflow-x-auto no-scrollbar -mx-4 px-4"><div className="flex gap-2 w-max">{filters.map((item) => <button key={item.value} onClick={() => setFilter(item.value)} className={cn('rounded-full border px-3 py-2 text-xs transition', filter === item.value ? 'bg-[#a7ff4f] border-[#a7ff4f] text-[#071006]' : 'bg-white/[0.04] border-white/10 text-[#cdd3c8]')}>{item.label}</button>)}</div></div><div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.04] px-3.5 py-3 text-xs text-[#d8ded2] mb-3"><span><Wallet className="inline h-3.5 w-3.5 mr-1.5"/>{visibleAccounts.length} shown</span><span className="font-mono">{hidden || money(visibleTotal, true)}</span></div><div className="space-y-0.5">{visibleAccounts.map((account) => { const Icon = typeIcon[account.type] ?? Landmark; const debt = ['loan','credit'].includes(account.type); const native = nativeDisplay(account); return <button onClick={() => openQuickUpdate(account)} key={account.id} className="relative w-full flex items-center justify-between rounded-2xl px-3.5 py-3 text-left hover:bg-white/[0.03]"><div className={cn('absolute left-0 top-3 bottom-3 w-1 rounded-full', debt ? 'bg-[#c96f5d]' : account.type === 'retirement' ? 'bg-[#31b8d8]' : 'bg-[#a7ff4f]')}/><div className="pl-3 min-w-0"><p className="text-sm truncate">{account.name}</p><span className="mt-1 inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/[0.04] px-2 py-0.5 text-[11px] text-[#8d9188]"><Icon className="h-3 w-3"/>{typeLabel[account.type] ?? account.type}</span></div><div className="text-right"><p className="text-sm shrink-0 font-medium">{hidden || money(Math.abs(Number(account.balance)))}</p>{native && <p className="text-[10px] text-[#8d9188]">{hidden || native}</p>}<p className="text-[10px] text-[#8d9188]">Tap to update</p></div></button>})}</div></section>}{tab === 'insights' && <section className="space-y-3.5"><div><h2 className="text-xl font-light">Smart Insights</h2><p className="text-[#a8aca3] text-xs">Based on your entered assets, liabilities, and snapshots.</p></div><div className="grid grid-cols-2 gap-3"><div className="rounded-[20px] bg-white/[0.05] border border-white/8 p-3.5"><Target className="h-5 w-5 text-[#a7ff4f] mb-2"/><p className="text-xs text-[#a8aca3]">Next milestone</p><p className="text-lg font-light">{hidden || money(nextMilestone, true)}</p><p className="text-[11px] text-[#8d9188]">{hidden || money(milestoneGap, true)} to go</p></div><div className="rounded-[20px] bg-white/[0.05] border border-white/8 p-3.5"><Trophy className="h-5 w-5 text-[#a7ff4f] mb-2"/><p className="text-xs text-[#a8aca3]">Largest position</p><p className="text-lg font-light truncate">{largestAccount?.name || 'None'}</p><p className="text-[11px] text-[#8d9188]">{largestAccount ? hidden || money(Math.abs(Number(largestAccount.balance)), true) : money(0)}</p></div></div><div className="rounded-[20px] bg-white/[0.05] border border-white/8 p-3.5"><h3 className="text-base mb-2">What drives net worth</h3><p className="text-xs text-[#a8aca3] mb-3">Largest category by current asset value.</p><div className="flex justify-between text-sm"><span>{topDriver?.label || 'No assets yet'}</span><span className="font-mono">{topDriver ? `${topDriver.pct.toFixed(1)}%` : '0%'}</span></div><div className="h-2 rounded-full bg-white/10 overflow-hidden mt-2"><div className="h-full rounded-full bg-[#a7ff4f]" style={{ width: `${Math.min(topDriver?.pct || 0, 100)}%` }}/></div></div><div className="rounded-[20px] bg-white/[0.05] border border-white/8 p-3.5"><Sparkles className="h-5 w-5 text-[#75efad] mb-2"/><h3 className="text-base mb-1">Forecast</h3><p className="text-xs text-[#a8aca3]">Simple forecast using snapshot growth when available, otherwise 6% placeholder growth.</p><p className="text-2xl font-light mt-2">{hidden || money(forecast)}</p></div><div className="rounded-[20px] bg-white/[0.05] border border-white/8 p-3.5"><h3 className="text-base mb-2">Calculation base</h3><div className="space-y-1.5 text-xs font-mono text-[#d8ded2]"><div className="flex justify-between"><span>Assets</span><span>{hidden || money(totals.assets, true)}</span></div><div className="flex justify-between"><span>Debt</span><span>{hidden || money(totals.liabilities, true)}</span></div><div className="flex justify-between"><span>Debt ratio</span><span>{debtRatio.toFixed(1)}%</span></div></div></div><div className="rounded-[20px] bg-white/[0.05] border border-white/8 p-3.5"><div className="flex items-center justify-between mb-3"><h3 className="text-base">Rule-based insights</h3><Lightbulb className="h-4 w-4 text-[#75efad]"/></div><div className="space-y-2.5">{recommendations.map((rec) => <div key={rec.title} className="rounded-2xl bg-black/20 border border-white/8 p-3"><div className="flex gap-2.5"><rec.icon className="h-4 w-4 text-[#75efad] shrink-0 mt-1"/><div><p className="font-semibold text-xs">{rec.title}</p><p className="text-[11px] text-[#a8aca3] leading-relaxed">{rec.body}</p></div></div></div>)}</div></div></section>}{tab === 'portfolio' && <section className="space-y-3.5"><div><h2 className="text-xl font-light">Asset Breakdown</h2><p className="text-[#a8aca3] text-xs">Real percentages from your account balances.</p></div><div className="rounded-[20px] bg-white/[0.05] border border-white/8 p-3.5 space-y-4">{breakdown.map((item) => <div key={item.label}><div className="flex justify-between text-sm mb-2"><span>{item.label}</span><span className="font-mono">{hidden || money(item.value, true)} · {item.pct.toFixed(1)}%</span></div><div className="h-2 rounded-full bg-white/10 overflow-hidden"><div className="h-full rounded-full bg-[#a7ff4f]" style={{ width: `${Math.min(item.pct, 100)}%` }}/></div></div>)}</div><div className="rounded-[20px] bg-white/[0.05] border border-white/8 p-3.5 flex justify-between"><span className="text-sm">Debt-to-asset ratio</span><b className="text-sm">{debtRatio.toFixed(1)}%</b></div></section>}</div></div><button onClick={() => router.push('/accounts')} className={cn('fixed right-5 z-[70] h-14 w-14 rounded-[18px] border-[3px] border-[#2f7dff] bg-[#a7ff4f] text-[#071006] shadow-2xl flex items-center justify-center transition-all duration-300', showAdd ? 'bottom-24 opacity-100 translate-y-0 scale-100' : 'bottom-8 opacity-0 translate-y-8 scale-90 pointer-events-none')}><Plus className="h-7 w-7"/></button><nav className="fixed left-4 right-4 bottom-5 z-50 mx-auto max-w-[640px] rounded-[24px] border border-white/15 bg-[#10140f]/90 backdrop-blur-xl p-1.5 shadow-2xl"><div className="grid grid-cols-3 gap-1.5">{(['accounts','insights','portfolio'] as Tab[]).map((item) => { const Icon = item === 'accounts' ? Wallet : item === 'insights' ? Waves : Sparkles; return <button key={item} onClick={() => setTab(item)} className={cn('h-11 rounded-[18px] flex items-center justify-center gap-2 font-semibold capitalize text-[#cdd3c8] text-xs', tab === item && 'bg-white/18 text-white')}><Icon className="h-4 w-4"/><span className="hidden sm:inline">{item}</span></button> })}</div></nav>{quickAccount && <div className="fixed inset-0 z-[90] bg-black/55 backdrop-blur-sm flex items-end"><div className="w-full max-w-[680px] mx-auto rounded-t-[28px] bg-[#11160f] border-t border-white/10 p-5 shadow-2xl max-h-[88vh] overflow-y-auto"><div className="flex items-start justify-between mb-5"><div><p className="text-xs text-[#a8aca3]">Quick update</p><h2 className="text-xl font-semibold">{quickAccount.name}</h2><p className="text-sm text-[#8d9188]">{quickAccount.institution}</p></div><button onClick={() => setQuickAccount(null)} className="h-10 w-10 rounded-full bg-white/[0.06] border border-white/10 flex items-center justify-center"><X className="h-5 w-5"/></button></div><label className="block text-sm text-[#a8aca3] mb-2">Date</label><input type="date" value={quickDate} onChange={(e) => setQuickDate(e.target.value)} className="w-full rounded-2xl bg-black/25 border border-white/10 px-4 py-3 outline-none mb-4"/><label className="block text-sm text-[#a8aca3] mb-2">Currency</label><select value={quickCurrency} onChange={(e) => { setQuickCurrency(e.target.value); if (e.target.value === 'MYR') { setQuickRate('1'); setQuickRateMeta('MYR base currency'); } }} className="w-full rounded-2xl bg-black/25 border border-white/10 px-4 py-3 outline-none mb-4">{currencies.map((item) => <option key={item} value={item}>{item}</option>)}</select><label className="block text-sm text-[#a8aca3] mb-2">Balance in {quickCurrency}</label><div className="flex items-center gap-3 rounded-2xl bg-black/25 border border-white/10 px-4 py-4 mb-4"><span className="text-[#a8aca3]">{quickCurrency}</span><input className="w-full bg-transparent outline-none text-2xl font-light" type="number" step="0.000001" value={quickNative} onChange={(e) => setQuickNative(e.target.value)}/></div><div className="flex items-center justify-between mb-2"><label className="block text-sm text-[#a8aca3]">MYR rate per 1 {quickCurrency}</label><button type="button" onClick={refreshQuickRate} disabled={quickCurrency === 'MYR' || quickRateLoading} className="rounded-full border border-white/10 bg-white/[0.05] px-3 py-1 text-xs text-[#d8ded2] disabled:opacity-40 flex items-center gap-1"><RefreshCw className="h-3 w-3"/>{quickRateLoading ? 'Refreshing' : 'Refresh'}</button></div><div className="flex items-center gap-3 rounded-2xl bg-black/25 border border-white/10 px-4 py-4 mb-2"><span className="text-[#a8aca3]">RM</span><input className="w-full bg-transparent outline-none text-2xl font-light" type="number" step="0.000001" value={quickRate} onChange={(e) => { setQuickRate(e.target.value); setQuickRateMeta('Manual override'); }} disabled={quickCurrency === 'MYR'}/></div><p className="text-xs text-[#8d9188] mb-4">{quickRateMeta}</p><div className="rounded-2xl border border-[#a7ff4f]/20 bg-[#a7ff4f]/10 px-4 py-3 mb-4"><p className="text-xs text-[#a8aca3]">MYR value used in net worth</p><p className="text-xl font-semibold">{money(quickMyr)}</p><p className="text-xs text-[#a8aca3] mt-1">{nativeAmount(quickNative, quickCurrency)} × RM{Number(quickRate || 0).toLocaleString()}</p></div><textarea value={quickNote} onChange={(e) => setQuickNote(e.target.value)} placeholder="Optional note" className="w-full min-h-20 rounded-2xl bg-black/25 border border-white/10 px-4 py-3 outline-none mb-4"/><div className="grid grid-cols-2 gap-3"><button onClick={() => router.push(`/accounts/${quickAccount.id}`)} className="rounded-2xl bg-white/[0.06] border border-white/10 py-4 font-semibold">View details</button><button onClick={saveQuickUpdate} disabled={quickSaving} className="rounded-2xl bg-[#a7ff4f] text-[#071006] py-4 font-bold disabled:opacity-60 flex items-center justify-center gap-2"><Save className="h-4 w-4"/>{quickSaving ? 'Saving...' : 'Save'}</button></div></div></div>}</div></main>;
}
