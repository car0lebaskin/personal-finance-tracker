...
import { BarChart3, Coins, CreditCard, Eye, EyeOff, Home, Landmark, Lightbulb, Plus, Save, Sparkles, Target, Trophy, UserCircle, Wallet, Waves, X } from 'lucide-react';
import { supabase } from '@/lib/supabase';
import { cn } from '@/lib/utils';
import { getBreakdown, getRecommendations, getTotals } from '@/lib/finance';
...
export default function VaultDashboard() {
  ...
  const chartPoints = useMemo(() => {
    ...
    const coords: NetWorthPoint[] = source.map((point, index) => ({ ...point, x: source.length === 1 ? 350 : 30 + (index * 640) / (source.length - 1), y: 130 - ((point.value - min) / range) * 90 }));
  ...
  function openQuickUpdate(account: Account) {
    ...
    setQuickBalance(String(account.balance || 0));
    setQuickDate(today());
    setQuickNote('');
  }
  ...
  async function saveQuickUpdate() {
    ...
    const newBalance = Number(quickBalance || 0);
    const update = await supabase.from('accounts').update({ balance: newBalance }).eq('id', quickAccount.id);
    if (update.error) { setQuickSaving(false); setStatus(update.error.message); return; }
    const cleanup = await supabase.from('account_snapshots').delete().eq('account_id', quickAccount.id).eq('snapshot_date', quickDate);
    if (cleanup.error) { setQuickSaving(false); setStatus(cleanup.error.message); return; }
    const insert = await supabase.from('account_snapshots').insert({ user_id: userId, account_id: quickAccount.id, snapshot_date: quickDate, balance: newBalance, notes: quickNote || 'Quick update' });
    setQuickSaving(false);
    if (insert.error) { setStatus(insert.error.message); return; }
