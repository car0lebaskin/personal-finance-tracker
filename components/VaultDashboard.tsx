...
import { BarChart3, Coins, CreditCard, Eye, EyeOff, Home, Landmark, Lightbulb, Plus, RefreshCw, Save, Sparkles, Trophy, UserCircle, Wallet, Waves, X } from 'lucide-react';
...
export default function VaultDashboard() {
  ...
  async function loadData(useCache = false) {
    ...
    if (!fullResult.error && fullResult.data) nextAccounts = fullResult.data as VaultAccount[];
      ...
      if (!basicResult.error && basicResult.data) nextAccounts = basicResult.data as VaultAccount[];
        ...
        if (!finalResult.error && finalResult.data) nextAccounts = finalResult.data as VaultAccount[];
      }
    }
  ...
  const latestSnapshotDate = fullTrend.at(-1)?.date || '';
  const staleAccounts = accounts.filter((account) => !snapshots.some((snap) => snap.account_id === account.id && snap.snapshot_date === latestSnapshotDate));
  ...
  }, [chartTrend, totals.netWorth, totals.assets]);
