...
const filters: { value: Filter; label: string; types?: string[] }[] = [
  { value: 'all', label: 'All' }, { value: 'cash', label: 'Cash', types: ['checking', 'savings', 'cash'] }, { value: 'investment', label: 'Invest', types: ['investment'] }, { value: 'crypto', label: 'Crypto', types: ['crypto'] }, { value: 'retirement', label: 'EPF', types: ['retirement'] }, { value: 'property', label: 'Property', types: ['property'] }, { value: 'loan', label: 'Debt', types: ['loan', 'credit'] },
];
...
export default function VaultDashboardCoach() {
  ...
  const [coachMode, setCoachMode] = useState('rules');
  ...
  async function loadData() {
    ...
    const history = await supabase.from('account_snapshots').select('account_id,snapshot_date,balance').order('snapshot_date', { ascending: true });
  ...
  const visibleAccounts = useMemo(() => { const selected = filters.find((item) => item.value === filter); return selected?.types ? accounts.filter((a) => selected.types!.includes(a.type)) : accounts; }, [accounts, filter]);
  const latestChange = useMemo(() => {
    const dates = Array.from(new Set(snapshots.map((s) => s.snapshot_date))).sort();
    if (dates.length < 2) return 0;
    const lastDate = dates[dates.length - 1];
    const prevDate = dates[dates.length - 2];
    const valueAt = (date: string) => accounts.reduce((sum, account) => { const rows = snapshots.filter((s) => s.account_id === account.id && s.snapshot_date <= date); const latest = rows[rows.length - 1]; const amount = latest ? Number(latest.balance) : Number(account.balance || 0); return sum + (['loan','credit'].includes(account.type) ? -Math.abs(amount) : amount); }, 0);
    return valueAt(lastDate) - valueAt(prevDate);
  }, [accounts, snapshots]);
  ...
  async function refreshCoach() {
    ...
    try {
      ...
      setCoach(data.actions || buildPortfolioCoach(accounts));
      setCoachMode(data.mode || 'rules');
    } catch { setCoach(buildPortfolioCoach(accounts)); setCoachMode('rules'); }
    setCoachLoading(false);
  }

  function openQuickUpdate(account: Account) {
    setQuickAccount(account); setQuickCurrency(account.currency || 'MYR'); setQuickNative(String(account.native_balance ?? account.balance ?? 0)); setQuickRate(String(account.fx_rate ?? 1)); setQuickDate(today()); setQuickNote(''); setQuickRateMeta(account.currency === 'MYR' ? 'MYR base currency' : 'Saved rate');
  }
  async function refreshQuickRate() {
    ...
    catch (err) { setStatus(err instanceof Error ? err.message : 'Unable to refresh rate.'); setQuickRateMeta('Manual rate'); }
  ...
  async function snapshotToday() {
    setSnapshotSaving(true); setStatus('');
    const session = await supabase.auth.getSession(); const userId = session.data.session?.user.id;
    if (!userId) { router.push('/login'); return; }
    const date = today(); const ids = accounts.map((a) => a.id);
    await supabase.from('account_snapshots').delete().eq('snapshot_date', date).in('account_id', ids);
    let result = await supabase.from('account_snapshots').insert(accounts.map((a) => ({ user_id: userId, account_id: a.id, snapshot_date: date, balance: Number(a.balance), native_balance: Number(a.native_balance ?? a.balance), currency: a.currency || 'MYR', fx_rate: Number(a.fx_rate ?? 1), notes: 'Daily snapshot' })));
    if (result.error && result.error.message.toLowerCase().includes('native_balance')) result = await supabase.from('account_snapshots').insert(accounts.map((a) => ({ user_id: userId, account_id: a.id, snapshot_date: date, balance: Number(a.balance), notes: 'Daily snapshot' })));
