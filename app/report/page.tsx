...
type Contribution = { id: string; date?: string; entry_date?: string; category: string; amount: number; note?: string };
type Goal = { id: string; name: string; category: string; target_amount: number; monthly_contribution: number };
...
function ReportContent() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const contributionResult = await supabase.from('contribution_entries').select('id,entry_date,category,amount,note').order('entry_date', { ascending: false }).limit(100);
      const goalResult = await supabase.from('goals').select('id,name,category,target_amount,monthly_contribution').limit(50);
      const recurringResult = await supabase.from('recurring_contributions').select('id,category,amount,active,last_run_month').eq('active', true).limit(50);
      const linkResult = await supabase.from('account_links').select('id,asset_account_id,liability_account_id,label').limit(50);
      ...
      if (!contributionResult.error && contributionResult.data) setContributions(contributionResult.data as Contribution[]); else setContributions(readContributions());
  ...
  async function generate() {
    setRunning(true);
    const response = await fetch('/api/monthly-report', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ accounts, snapshots, contributions, goals, recurring, links }) });
