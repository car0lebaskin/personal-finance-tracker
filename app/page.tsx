...
import { BarChart3, Coins, CreditCard, Home, Landmark, Lightbulb, Plus, Sparkles, UserCircle, Wallet, Waves } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const chartPoints = useMemo(() => {
    ...
    const coords = source.map((point, index) => ({ ...point, x: source.length === 1 ? 350 : 30 + (index * 640) / (source.length - 1), y: 130 - ((point.value - min) / range) * 90 }));
    ...
    <main className="min-h-screen bg-[#080b08] text-[#f4f5ef]">
      <div className="mx-auto max-w-[680px] min-h-screen relative overflow-hidden pb-32">
        ...
        <div className="relative px-4 pt-6">
          <header className="flex items-center justify-between mb-6"><h1 className="text-xl font-semibold tracking-tight">{tab === 'accounts' ? 'Vault' : tab === 'insights' ? 'Insights' : 'Portfolio'}</h1><button onClick={() => router.push('/accounts/profile')}><UserCircle className="h-8 w-8" /></button></header>

          {tab === 'accounts' && <section>
            <p className="text-[#a8aca3] text-xs mb-1.5">Net worth</p><div className="text-[1.7rem] leading-tight font-light tracking-tight mb-2">{money(totals.netWorth)}</div><p className="text-[11px] text-[#a8aca3] mb-4">{netWorthHistory.length ? 'Based on saved account snapshots' : 'Add dated updates to build history'}</p>
            <div className="h-[140px] -mx-4 mb-4 border-b border-white/10 relative"><svg viewBox="0 0 700 140" className="h-full w-full"><defs><linearGradient id="line" x1="0" x2="1"><stop offset="0%" stopColor="#35bdf5"/><stop offset="100%" stopColor="#69f0c2"/></linearGradient></defs><path d={chartPoints.path} fill="none" stroke="url(#line)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />{chartPoints.coords.map((p) => <circle key={p.date} cx={p.x} cy={p.y} r="4" fill="#69f0c2" />)}</svg></div>
