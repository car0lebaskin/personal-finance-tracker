...
export default function DashboardPage() {
  ...
  const chartPoints = useMemo(() => {
    ...
    const coords: NetWorthPoint[] = source.map((point, index) => ({
      ...point,
      x: source.length === 1 ? 350 : 30 + (index * 640) / (source.length - 1),
      y: 130 - ((point.value - min) / range) * 90,
    }));
  ...
  async function snapshotToday() {
    ...
    const date = today();
    const rows = accounts.map((account) => ({ user_id: userId, account_id: account.id, snapshot_date: date, balance: Number(account.balance), notes: 'Monthly snapshot' }));
    const result = await supabase.from('account_snapshots').insert(rows);
    setSnapshotSaving(false);
    if (result.error) { setStatus(result.error.message); return; }
    setStatus(`Snapshot saved for ${accounts.length} accounts.`);
    await loadData();
  }
  ...
  return (
    <main className="min-h-screen bg-[#080b08] text-[#f4f5ef]">
      <div className="mx-auto max-w-[680px] min-h-screen relative overflow-hidden pb-32">
        ...
        <div className="relative px-4 pt-6">
          ...
          {tab === 'accounts' && <section>
            ...
            <div className="text-[1.7rem] leading-tight font-light tracking-tight mb-2">{money(totals.netWorth)}</div>
            <p className="text-[11px] text-[#a8aca3] mb-4">{netWorthHistory.length ? 'Based on saved account snapshots' : 'Add dated updates to build history'}</p>
            <div className="h-[170px] -mx-4 mb-4 border-b border-white/10 relative">
              <svg viewBox="0 0 700 150" className="h-full w-full">
                <defs><linearGradient id="line" x1="0" x2="1"><stop offset="0%" stopColor="#35bdf5"/><stop offset="100%" stopColor="#69f0c2"/></linearGradient></defs>
                <path d={chartPoints.path} fill="none" stroke="url(#line)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
                {chartPoints.coords.map((p, index) => <circle key={`${p.date}-${index}`} onClick={() => setActiveChartIndex(index)} cx={p.x} cy={p.y} r={index === selectedIndex ? 7 : 4} fill={index === selectedIndex ? '#f4f5ef' : '#69f0c2'} className="cursor-pointer" />)}
              </svg>
              {selectedPoint && <div className="absolute left-4 top-3 rounded-2xl bg-black/45 border border-white/10 px-3 py-2 backdrop-blur"><p className="text-[11px] text-[#a8aca3]">{prettyDate(selectedPoint.date)}</p><p className="text-sm font-mono">{money(selectedPoint.value)}</p><p className={selectedChange >= 0 ? 'text-[11px] text-[#75efad]' : 'text-[11px] text-red-200'}>{previousPoint ? `${selectedChange >= 0 ? '+' : ''}${money(selectedChange)} (${selectedPct.toFixed(1)}%)` : 'First snapshot'}</p></div>}
            </div>
            <button onClick={snapshotToday} disabled={snapshotSaving} className="w-full rounded-2xl bg-[#a7ff4f] text-[#071006] font-semibold text-sm py-3 mb-2 disabled:opacity-60">{snapshotSaving ? 'Saving snapshot...' : 'Snapshot today'}</button>
            {status && <p className="text-xs text-[#a8aca3] mb-3">{status}</p>}
            ...
            <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.04] px-3.5 py-3 text-xs text-[#d8ded2] mb-3"><span><Wallet className="inline h-3.5 w-3.5 mr-1.5" />{visibleAccounts.length} shown</span><span className="font-mono">{money(visibleTotal, true)}</span></div>
